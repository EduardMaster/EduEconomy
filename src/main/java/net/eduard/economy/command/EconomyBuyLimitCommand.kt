...
class EconomyBuyLimitCommand : CommandManager("buylimit","limitedecompra") {
    init{
        usage = "/money buylimit [player]"
        description = "Ver Limite de Compra do jogador"
    }

    override fun command(sender: CommandSender, args: Array<String>) {
        if (args.isEmpty()){
            if (sender is Player) {
                sender.sendMessage(EduEconomyPlugin.instance.message("buy-limit-check")
